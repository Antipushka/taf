...
public abstract class AbstractAutomat<T extends AbstractAutomat<T>> implements Regulars {
    ...
    core.automat.State state;
    Stack<String> input;
    List<core.automat.AutomatConfiguration> trace;
    boolean abortIncorrectInput;

    AbstractAutomat() {
        counter = new AtomicInteger(0);
        state = core.automat.State.INITIAL;
        input = new Stack<>();
    ...
    public T grammar(Grammar grammar) {
        ...
        if(grammar.classifiedAs(TYPE_0, CONTEXT_DEPENDANT)) {
            throw new core.automat.InvalidGrammarException("Автомат не принимает грамматику с типом ниже КС (тип 2)");
        }
        return (T) this;
    }
    ...
    void track(int number, String next, String magazineContent) {
        trace.add(new core.automat.AutomatConfiguration(number,
                                           next,
                                           input,
                                           magazineContent,
                                           state.getLiteral()));
